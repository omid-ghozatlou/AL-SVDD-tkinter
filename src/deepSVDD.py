...
from networks.galNet import build_network, build_autoencoder
...
class DeepSVDD(object):
    ...
    def test(self, export_file, dataset: BaseADDataset, device: str = 'cuda', n_jobs_dataloader: int = 0):
        ...
        for labels, scores in tuple(zip(self.trainer.labels, self.trainer.scores)):
            out_file.write('%d \t %.6f \n' %(labels, scores))
        out_file.close()
    ...
    def apply_model(self, export_file, dataset: BaseADDataset, device: str = 'cuda', n_jobs_dataloader: int = 0):
        """Applies autoencoder and Deep SVDD models on unlabelled data"""
        # self.trainer = DeepSVDDTrainer(self.objective, self.R, self.c, self.nu,
        #                                device=device, n_jobs_dataloader=n_jobs_dataloader)
        idx=self.trainer.apply_model(dataset, self.net)
        # self.idx_sorted = []
        out_file = open(export_file,'w')
